...
export default function GuideMode({ isAdminStudio = false, selectedTechniqueName = "" }) {
  ...
  return <section className="guide-mode" ref={guideRef}>
    ...
    {quiz.length ? <section className="guide-mode__knowledge"><p className="eyebrow">Quick knowledge check</p><h2>Before you practise</h2>{quiz.slice(0, 5).map((question, index) => { const answer = quizAnswers[index]; return <article key={question.id || index}><strong>{question.question}</strong><div>{list(question.options).map((option) => { const label = option.label || option.text || option; const isSelected = answer === (option.id || label); return <button aria-pressed={isSelected} className={isSelected ? (option.correct ? "is-correct" : "is-incorrect") : ""} key={option.id || label} onClick={() => setQuizAnswers((current) => ({ ...current, [index]: option.id || label }))} type="button">{label}</button>; })}</div>{answer ? <small>{question.feedback || (list(question.options).find((option) => (option.id || option.label || option.text || option) === answer)?.correct ? "Correct." : "Review the phase cues and try again.")}</small> : null}</article>; })}</section> : null}
